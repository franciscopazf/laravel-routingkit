...
class FpFNavigation extends FpFBaseEntity
{
    ...
    {
        if (FpFRoute::exists($this->instanceRouteId)) {
    ...
    public function loadFromFpRoute(): self
    {
        ...
        $route = FpFRoute::findById($this->instanceRouteId);
    ...
    public function FpFRoute(): ?FpFEntityInterface
    {
        if (FpFRoute::exists($this->instanceRouteId)) {
            return FpFRoute::findById($this->instanceRouteId);
        }
        return null;
    }
    ...
    public static function createConsoleAtributte(array $data): array
    {
        $attributes =  [
            ...
            'id' => [
                ...
                'rules' => [
                    ...
                    'expect_false' => function($value) {
                        return FpFNavigation::exists($value);
                    }
                ],
                'closure' => function($parametros){
                    if (!FpFNavigation::exists($parametros['instanceRouteId'])) {
            ...
            'parentId' => [
                ...
                'closure' => fn() => FpFNavigation::seleccionar(null, 'Selecciona el padre de la ruta'),
            ]
        ];
    ...
    public function getOmmittedAttributes(): array
    {
        return [
            ...
            'url' => ['same:FpFRoute().url', 'isTrue:isGroup'],

            'makerMethod' => ['omit'],
            'urlName' => ['same:instanceRouteId'],
            'accessPermission' => ['same:FpFRoute().accessPermission'],
