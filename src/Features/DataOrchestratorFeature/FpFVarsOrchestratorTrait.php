...
trait FpFVarsOrchestratorTrait
{
     ...
     *
     * @param Collection $flat A flattened collection of FpFEntityInterface.
     ...
     *
     * @param FpFEntityInterface $currentNode The current node to build items for.
     * @param Collection $allEntitiesFlattened All entities in flattened form (from the source tree).
     */
    protected function buildSubTreeRecursive(FpFEntityInterface $currentNode, Collection $allEntitiesFlattened): void
    {
     ...
     */
    public function getBreadcrumbs(FpFEntityInterface $entity): Collection
    {
     ...
     * @return FpF\RoutingKit\Contracts\FpFEntityInterface|null The root entity of the active branch, or null if not found.
     */
    public function getActiveBranch(?string $activeRouteName = null): ?FpFEntityInterface
    {
    ...
    public function getBreadcrumbsForCurrentRoute(?string $activeRouteName = null): Collection
    {
        ...
        // **CORRECCIÓN:** Asegurarse de que la entidad obtenida sea del tipo correcto FpFEntityInterface.
        ...
        if (!$activeEntity instanceof FpFEntityInterface) {
            return collect(); // No se encontró una entidad activa válida o no es del tipo correcto.
        }

        // Llama al método getBreadcrumbs con la entidad FpFEntityInterface.
    ...
    // abstract protected function getContextInstance(string $contextKey): FpFEntityInterface; // O la interfaz de tu contexto.
}
