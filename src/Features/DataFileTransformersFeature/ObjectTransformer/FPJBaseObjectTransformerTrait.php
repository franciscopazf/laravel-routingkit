...
use FPJ\RoutingKit\Services\Route\Strategies\RouteContentManager;
use FPJ\RoutingKit\Contracts\FPJileTransformerInterface;
use FPJ\RoutingKit\Features\DataValidationsFeature\FPJAttributeOmitter;
use Illuminate\Support\Collection;
use Illuminate\Support\Str;
use ReflectionClass;


trait FPJBaseObjectTransformerTrait
{
    ...
    public function getBlock(FPJEntityInterface $entity): string
    {
    ...
    public function getLevelIdent(FPJEntityInterface $entity): int
    {
    ...
    public function sanitizeForArray(string $block, FPJEntityInterface $entity): string
    {
    ...
    public function insertItem(string $block, string $Item, FPJEntityInterface $entity, int $level): string
    {
    ...
    private function getBlockFromFile(FPJEntityInterface $entity): string
    {
    ...
    private function rebuildRouteContent(FPJEntityInterface $entity, bool $setParent = false): string
    {
        ...
        $validator = FPJAttributeOmitter::make(object: $entity);
     ...
     *
     * @param FPJEntityInterface $entity La entidad de la cual se obtendrán las propiedades y la clase.
     * @return string La cadena de código literal para el método 'make'.
     */
    public function getMakerCallLiteral(FPJEntityInterface $entity): string
    {
     ...
     */
    public function getMakerPattern(FPJEntityInterface $entity): string
    {
        $reflectionClass = new ReflectionClass($entity);
        $className = $reflectionClass->getShortName();
        $makerMethodName = $entity->getProperties()['makerMethod'] ?? 'make';
    ...
    // retorna el patron que permite buscar rutas
    // en el archivo de rutas.
    private function getBlockPattern(FPJEntityInterface $entity): string
    {
