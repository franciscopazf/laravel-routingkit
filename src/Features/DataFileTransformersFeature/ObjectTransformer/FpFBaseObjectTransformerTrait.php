...
use FpF\RoutingKit\Services\Route\Strategies\RouteContentManager;
use FpF\RoutingKit\Contracts\FpFileTransformerInterface;
use FpF\RoutingKit\Features\DataValidationsFeature\FpFAttributeOmitter;
use Illuminate\Support\Collection;
use Illuminate\Support\Str;
use ReflectionClass;


trait FpFBaseObjectTransformerTrait
{
    ...
    public function getBlock(FpFEntityInterface $entity): string
    {
    ...
    public function getLevelIdent(FpFEntityInterface $entity): int
    {
    ...
    public function sanitizeForArray(string $block, FpFEntityInterface $entity): string
    {
    ...
    public function insertItem(string $block, string $Item, FpFEntityInterface $entity, int $level): string
    {
    ...
    private function getBlockFromFile(FpFEntityInterface $entity): string
    {
    ...
    private function rebuildRouteContent(FpFEntityInterface $entity, bool $setParent = false): string
    {
        ...
        $validator = FpFAttributeOmitter::make(object: $entity);
     ...
     *
     * @param FpFEntityInterface $entity La entidad de la cual se obtendrán las propiedades y la clase.
     * @return string La cadena de código literal para el método 'make'.
     */
    public function getMakerCallLiteral(FpFEntityInterface $entity): string
    {
     ...
     */
    public function getMakerPattern(FpFEntityInterface $entity): string
    {
        $reflectionClass = new ReflectionClass($entity);
        $className = $reflectionClass->getShortName();
        $makerMethodName = $entity->getProperties()['makerMethod'] ?? 'make';
    ...
    // retorna el patron que permite buscar rutas
    // en el archivo de rutas.
    private function getBlockPattern(FpFEntityInterface $entity): string
    {
